...
The Self.xyz contract address is {os.getenv("SELF_CONTRACT_ADDRESS")}.
The relevant chain is Celo (https://alfajores.celoscan.io)
...
You are assigned the role of Alice's agent.
...
async def handle_post(ctx: Context, req: Request) -> Response:
    ...
    history += f"Alice asked: {req.text}\n\n"
    ans = prompt_as1(f"{history}\n\Before you answer to Alice, do you need to ask something to Bob's agent (CAUTION: only do this if absolutely necessary as this has high overhead)? (if yes, just write your question, if not, write 'no')")
    if len(ans) > 10:

        reply, status = await ctx.send_and_receive(req.bob_address, ans, response_type=Message)
        if isinstance(reply, Message):
            history += f"I (Alice's agent) asked Bob's agent: {ans}\n\n"
            history += f"Bob's agent answered: {reply.message}\n\n"
        else:
            ctx.logger.info(f"Failed to receive response from bob: {status}")
    ans = prompt_as1(f"{history}\n\nWhat is your answer to Alice? (Just write the answer)")
    history += f"I (Alice's agent) answered: {ans}\n\n"
