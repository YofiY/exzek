...
const IdentityModal = memo(function IdentityModal({ 
  isOpen, 
  onClose, 
  onComplete, 
  selfApp, 
...
const AIChatPage = memo(function AIChatPage() {
  ...
  const handleSuccessfulVerification = (data: any) => {
    ...
    displayToast("Identity verification successful!");
    
    // Add success message to chat
    const successMessage: Message = {
      id: Date.now().toString(),
      type: 'ai',
      content: "🎉 Identity verification completed successfully! Your verified information has been securely processed.",
      timestamp: new Date().toISOString()
    };
  ...
  const handleVerificationError = (error: string) => {
    ...
    const errorMessage: Message = {
      id: Date.now().toString(),
      type: 'ai',
      content: "❌ Identity verification failed. Please try again or contact support if the issue persists.",
      timestamp: new Date().toISOString()
    };
  ...
  const handleIdentityVerification = (selectedOptions: IdentityOption[]) => {
    ...
    if (!user?.walletAddress) {
      displayToast("Please connect your wallet first to verify your identity");
      return;
    }
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: qrCodeStyles }} />
        ...
        <div style={{
          ...
          <BoxReveal boxColor="#3b82f6" duration={0.3}>
            ...
            justifyContent: 'center',
              ...
              <div className="flex gap-3 items-end" style={{ 
            display: 'flex',
            justifyContent: 'center',
              alignItems: 'center',
